...
public class SellerServiceImpl implements SellerService {
	 ...
	 */
	@Override
	public void updatePassword(TbSeller seller,String sellerId,String password){
		TbSeller oldSeller = sellerMapper.selectByPrimaryKey(sellerId);
		boolean pswFlag = BCrypt.checkpw(password,oldSeller.getPassword());
		if(pswFlag){
			//密码加密
			BCryptPasswordEncoder passwordEncoder=new BCryptPasswordEncoder();
			String newPassword = passwordEncoder.encode(seller.getPassword());//加密
			seller.setSellerId(sellerId);
			seller.setPassword(newPassword);
			int key = sellerMapper.updateByPrimaryKeySelective(seller);
